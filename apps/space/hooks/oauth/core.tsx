...
export const useCoreOAuthConfig = (oauthActionText: string): TOAuthConfigs => {
  ...
  const oAuthOptions: TOAuthOption[] = [
    ...
    {
      id: "authentik",
      text: `${oauthActionText} with Authentik`,
      icon: <img src={authentikLogo} height={18} width={18} alt="Authentik Logo" />,
      onClick: () => {
        window.location.assign(`${API_BASE_URL}/auth/authentik/${next_path ? `?next_path=${next_path}` : ``}`);
      },
      enabled: config?.is_authentik_enabled,
    },
  ];
